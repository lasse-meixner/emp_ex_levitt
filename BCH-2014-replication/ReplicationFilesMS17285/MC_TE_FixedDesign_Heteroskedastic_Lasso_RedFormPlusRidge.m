...
%
function [ ALL_StdErr, ALL_ALPHA ] = MC_TE_FixedDesign_Heteroskedastic_Lasso_RedFormPlusRidge ( NUM_SIM, rho, alpha0, R21, R22, design, p, n )
...
for k = 1 : NUM_SIM
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%       
        %% Get First Stage Ridge Fit using gamma*I_p in Tikhonov by 10-fold CV
        XRc = X(:,3:end)-ones(n,1)*mean(X(:,3:end));
        YRc = X(:,2)-mean(X(:,2));
        gamma = fminunc('CVridgeK',var(YRc)*n,optimset('disp','off'),...
            XRc,YRc,10);
        RidgeFit = mean(X(:,2))+XRc*((XRc'*XRc + gamma*eye(p))\(XRc'*YRc));
        
        X = [X RidgeFit]; %#ok<AGROW>
        ...
        %%% 3.  Run Y on D, X1 and X2.
        [ betaLASSO_1, ~] = MC_TE_LassoHeteroskedastic_unpenalized( X(:,2), [ X(:,1) X(:,1) X(:,3:(p+3))], conf_lvl, psi, 5, 1 );
        [ betaLASSO_2, ~] = MC_TE_LassoHeteroskedastic_unpenalized(Y, [ X(:,1) X(:,1) X(:,3:(p+3))],  conf_lvl, psi, 5, 1 );
...
%         sNEWSECOND_11 = 2 + max(size(IND_NEW));
        ...
        ORACLEsecond = [abs(beta0second) >= sqrt(var_noise_second/n) ; 0];
...
%         sNEWORACLE =  2 + sum(ORACLEsecond);       
        ...
        ORACLEfirst = [abs(beta0first) >= sqrt(var_noise_second/n) ; 0];
